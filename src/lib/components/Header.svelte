...
<header class="relative overflow-hidden rounded-2xl bg-teal-primary text-white shadow-sm">
	...
	<div
		class="relative flex flex-col gap-6 p-8 md:flex-row md:items-center md:gap-8 md:px-8 md:py-10"
	>
		<div
		...
		>
			<div class="text-xl leading-none font-semibold tracking-tight md:text-2xl">
				{identity.firstName}
			</div>
			<div class="text-xl leading-none font-semibold tracking-tight md:text-2xl">
				{identity.lastName}
		...
		<div class="min-w-0 flex-1">
			...
			<div
				class="mt-5 grid grid-cols-1 gap-x-4 gap-y-2 text-sm sm:grid-cols-2 lg:flex lg:flex-wrap lg:items-center"
			>
				<span class="flex items-center gap-2">
					<Mail class="h-4 w-4 opacity-80" aria-hidden="true" />
					<a class="underline-offset-2 hover:underline" href={`mailto:${identity.email}`}
						>{identity.email}</a
					>
				</span>
				<span class="flex items-center gap-2">
					...
					<a
						...
						href={`tel:${identity.phone.replace(/\s+/g, '')}`}>{identity.phone}</a
					>
				</span>
				{#each identity.links as link (link.url)}
					{@const brand = brandFor(link.url)}
					<a
						class="inline-flex items-center gap-2 underline-offset-2 opacity-90 hover:underline hover:opacity-100"
						href={link.url}
